function createBoardCardHtml(board){let roleLabel='';switch(board.currentUserRole){case 'board_member_manager':roleLabel='người quản lý';break;case 'board_editor':roleLabel='người chỉnh sửa';break;case 'board_viewer':roleLabel='người xem';break;default:roleLabel='người sở hữu'}
return `
    ...
    `}
$(document).ready(function(){$.ajaxSetup({headers:{'X-CSRF-TOKEN':$('meta[name="csrf-token"]').attr('content')}});$('#add-board-btn').on('click',function(){const boardName=prompt("Nhập tên bảng mới:","");if(boardName&&boardName.trim()!==""){$.ajax({url:window.routeUrls.boardsStore,method:'POST',data:{name:boardName.trim()},dataType:'json',success:function(response){if(response.success&&response.board){const newBoardHtml=createBoardCardHtml(response.board);$('#no-boards-message').remove();$('#board-list-container').prepend(newBoardHtml);alert(response.message)}else{alert(response.message||'Không thể tạo bảng. Phản hồi không hợp lệ.')}},error:function(xhr){console.error("Create Error XHR:",xhr);let errorMessage='Đã xảy ra lỗi khi tạo bảng.';if(xhr.responseJSON){if(xhr.responseJSON.errors&&xhr.responseJSON.errors.name){errorMessage=xhr.responseJSON.errors.name[0]}else if(xhr.responseJSON.message){errorMessage=xhr.responseJSON.message}}else{errorMessage=`Lỗi không xác định (HTTP ${xhr.status}: ${xhr.statusText})`}
console.error("Error response Text:",xhr.responseText);alert(errorMessage)}})}else if(boardName!==null){alert("Tên bảng không được để trống.")}});$('#board-list-container').on('click','.rename-board-link',function(e){e.preventDefault();const boardId=$(this).data('id');const currentName=$(this).data('name');const updateUrl=$(this).data('update-url');$('#renameBoardModal').find('#rename-board-id').val(boardId);$('#renameBoardModal').find('#rename-board-current-name').val(currentName);$('#renameBoardModal').find('#rename-board-new-name').val(currentName);$('#renameBoardModal').find('#rename-board-form').attr('action',updateUrl);$('#renameBoardModal').modal('show');$('#renameBoardModal').on('shown.bs.modal',function(){$('#rename-board-new-name',this).focus().select()}).on('hidden.bs.modal',function(){$(this).off('shown.bs.modal')})});$('#rename-board-form').on('submit',function(e){e.preventDefault();const boardId=$('#rename-board-id').val();const newName=$('#rename-board-new-name').val().trim();const currentName=$('#rename-board-current-name').val();const updateUrl=$(this).attr('action');if(newName&&newName!==currentName){performRenameAjax(boardId,newName,updateUrl)}else if(!newName){alert("Tên bảng không được để trống.")}else{$('#renameBoardModal').modal('hide')}});function performRenameAjax(boardId,newName,updateUrl){$.ajax({url:updateUrl,method:'PUT',data:{name:newName},dataType:'json',success:function(response){console.log("Update Success Response:",response);if(response.success){const $card=$(`#board-card-${boardId}`);$card.find('.board-name').text(response.new_name);$card.find('.rename-board-link').data('name',response.new_name);$card.find('.delete-board-link').data('name',response.new_name);if(response.updated_at_formatted){$card.find('.board-timestamp span').text(response.updated_at_formatted)}
$('#renameBoardModal').modal('hide')}else{alert(response.message||'Không thể đổi tên bảng.')}},error:function(xhr){console.error("Update Error XHR:",xhr);let errorMessage='Đã xảy ra lỗi khi đổi tên bảng.';if(xhr.responseJSON){if(xhr.responseJSON.errors&&xhr.responseJSON.errors.name){errorMessage=xhr.responseJSON.errors.name[0]}else if(xhr.responseJSON.message){errorMessage=xhr.responseJSON.message}}else{errorMessage=`Lỗi không xác định (HTTP ${xhr.status}: ${xhr.statusText})`}
console.error("Error response Text:",xhr.responseText);alert(errorMessage)}})}
$('#board-list-container').on('click','.delete-board-link',function(e){e.preventDefault();const boardId=$(this).data('id');const boardName=$(this).data('name');const destroyUrl=$(this).data('destroy-url');if(confirm(`Bạn có chắc chắn muốn xoá bảng "${boardName}" không? Hành động này không thể hoàn tác.`)){$.ajax({url:destroyUrl,method:'DELETE',dataType:'json',success:function(response){console.log("Delete Success Response:",response);if(response.success){$(`#board-card-${boardId}`).remove();if($('#board-list-container .board-card').length===0){$('#board-list-container').html(`
                                 ...
                                 `)}
alert(response.message)}else{alert(response.message||'Không thể xoá bảng.')}},error:function(xhr){console.error("Delete Error XHR:",xhr);let errorMessage='Đã xảy ra lỗi khi xoá bảng.';if(xhr.responseJSON&&xhr.responseJSON.message){errorMessage=xhr.responseJSON.message}else{errorMessage=`Lỗi không xác định (HTTP ${xhr.status}: ${xhr.statusText})`}
console.error("Error response Text:",xhr.responseText);alert(errorMessage)}})}});let isDragging=!1;let offsetX=0,offsetY=0;let $sheepWrapper=$(".sheep-wrapper");if($sheepWrapper.length){let originalPosition={top:$sheepWrapper.css("top"),left:$sheepWrapper.css("left")};if(typeof $.fn.draggable==='function'){$sheepWrapper.draggable({containment:"window",cursor:"grabbing",start:function(event,ui){isDragging=!0},stop:function(event,ui){isDragging=!1;$sheepWrapper.css("cursor","grab");let sheepOffset=ui.helper.offset();let sheepWidth=ui.helper.outerWidth();let sheepHeight=ui.helper.outerHeight();$("#board-list-container .card-drop-target").each(function(){let $card=$(this);let cardOffset=$card.offset();let cardWidth=$card.outerWidth();let cardHeight=$card.outerHeight();let collision=!(sheepOffset.left+sheepWidth<cardOffset.left||sheepOffset.left>cardOffset.left+cardWidth||sheepOffset.top+sheepHeight<cardOffset.top||sheepOffset.top>cardOffset.top+cardHeight);if(collision){let boardId=$card.attr("id").replace("board-card-","");showOverlayAndHideSheep(boardId);return!1}})}})}else{console.warn("jQuery UI Draggable not loaded. Sheep cannot be dragged.")}
function showOverlayAndHideSheep(boardId){$sheepWrapper.hide();if($("#cardOverlay").length===0){$('body').append('<div id="cardOverlay" style="display: none;"></div>')}
$.ajax({url:window.routeUrls.boardOverview.replace(':boardIdPlaceholder',boardId),method:'GET',success:function(response){if(response.success){const board_name=response.board_name;const assignees=response.assignees;const totalColumns=response.total_columns;const totalTasks=response.total_tasks;const assigneeRows=assignees.map(user=>`
                            <tr>
                                <td><img src="${user.avatar_url}" alt="${user.name}" class="rounded-circle" width="30" height="30"></td>
                                <td>${user.name}</td>
                                <td>${user.email}</td>
                            </tr>
                        `).join('');const overlayHtml=`
                        ...
                        `;$("#cardOverlay").html(overlayHtml);requestAnimationFrame(function(){const ovlContent=$("#ovl_content");const ovlTop=window.innerHeight/2-ovlContent.outerHeight()/2;const ovlLeft=window.innerWidth/2-ovlContent.outerWidth()/2;ovlContent.css({position:"fixed",top:ovlTop+"px",left:ovlLeft+"px",zIndex:1050});ovlContent.draggable({handle:"#dragHeader"})});$("#cardOverlay").fadeIn()}else{alert("Không lấy được dữ liệu tổng quan.")}},error:function(){alert("Có lỗi xảy ra khi gọi API.")}})}
$(document).on("click","#close",function(){$("#cardOverlay").fadeOut(function(){$sheepWrapper.css({top:originalPosition.top,left:originalPosition.left}).show()})});$sheepWrapper.on("click",function(){if(isDragging)return;const $sheep=$(this).find(".sheep");if($sheep.length===0)return;$sheep.stop(!0,!0).css("transform","rotate(0deg)");$({deg:0}).animate({deg:360},{duration:600,easing:'linear',step:function(now){$sheep.css("transform","rotate("+now+"deg)")},complete:function(){$sheep.css("transform","none");const $mouth=$sheep.find(".mouth");if($mouth.length){$mouth.addClass("yawn");setTimeout(()=>$mouth.removeClass("yawn"),800)}}})});const spinInterval=setInterval(function(){const $sheepForInterval=$(".sheep-wrapper .sheep");if($sheepForInterval.length===0){return}
