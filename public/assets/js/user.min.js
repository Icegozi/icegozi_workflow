...
$.ajax({url:window.routeUrls.boardOverview.replace(':boardIdPlaceholder',boardId),method:'GET',success:function(response){if(response.success){const assignees=response.assignees;const totalColumns=response.total_columns;const totalTasks=response.total_tasks;const assigneeRows=assignees.map(user=>`
                            <tr>
                        ...
                        `;$("#cardOverlay").html(overlayHtml);$("#ovl_content").css({position:"absolute",top:ovlTop,left:ovlLeft});$("#cardOverlay").fadeIn()}else{alert("Không lấy được dữ liệu tổng quan.")}},error:function(){alert("Có lỗi xảy ra khi gọi API.")}})}
